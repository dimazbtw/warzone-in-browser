...
export const KEYMAP = {
  ...
  reload: 'KeyR', primary: 'Digit1', secondary: 'Digit2', plate: 'KeyQ', heal: 'KeyH',
...
export const KEY_LABELS = {
  forward: 'Frente', back: 'Trás', left: 'Esquerda', right: 'Direita', sprint: 'Correr', crouch: 'Agachar', prone: 'Deitar (prone)',
  jump: 'Pular / mantle / paraquedas', interact: 'Pegar / abrir baú / segurar p/ reviver', reload: 'Recarregar',
  primary: 'Arma primária', secondary: 'Arma secundária', plate: 'Placa de armadura (segure p/ várias)', heal: 'Adrenalina (cura)',
  lethal: 'Granada', tactical: 'Granada de fumaça', melee: 'Faca / finalizar abatido', ping: 'Marcar (ping) · botão do meio',
...
const NAMES = { Space: 'Espaço', ShiftLeft: 'Shift', ControlLeft: 'Ctrl', Tab: 'Tab', BracketRight: ']', BracketLeft: '[', Escape: 'Esc' };
...
export class InputSystem extends EventTarget {
  constructor(canvas) {
    ...
    this.byCode = new Map();
    for (const [action, keys] of Object.entries(KEYMAP)) for (const k of [].concat(keys)) this.byCode.set(k, action);
